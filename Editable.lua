...
local MainWindow = WindUI:CreateWindow({
    ...
    ["User"] = {
        ...
        ["Callback"] = function()
            ...
            SendNexoraNotification("Theme Changed", "Switched to ", 2, "palette")
        end
    }
})
...
local function SaveConfig()
    ...
    SendNexoraNotification("Saved", "Layout saved!", 3, "check")
end
...
ButtonActions["KillAll_Btn"] = function()
    ...
    if not knife then
        SendNexoraNotification("Kill All", "You are not the Murderer", 3, "x")
        return
    end
    local myRoot = character:FindFirstChild("HumanoidRootPart")
...
ButtonActions["Gun_Btn"] = function()
    ...
    SendNexoraNotification("GUN", found and "Gun acquired" or "No gun found", 3, found and "check" or "x")
end
...
ButtonActions["Run_Btn"] = function()
    ...
    if character and character:FindFirstChild("Humanoid") then
        ...
        SendNexoraNotification("RUN", "Speed Activated", 3, "run")
        task.delay(5, function()
            if humanoid and humanoid.Parent then
                humanoid.WalkSpeed = humanoid.WalkSpeed - 9
                SendNexoraNotification("RUN", "Speed reverted", 3, "run")
            end
        end)
    end
end
...
createShape(
    ...
    function()
        ...
        local Gun = Backpack:FindFirstChild("Gun") or Character:FindFirstChild("Gun")
        if not Gun then 
            SendNexoraNotification("Shoot", "No Gun found", 3, "x")
            return 
        end
        ...
        if not Murderer or not Murderer.Character then 
            SendNexoraNotification("Shoot", "Murderer not found", 3, "x")
            return 
        end
...
getgenv()._AimToggle = function()
    ...
    if AimbotActive then
        StartAimbot()
        SendNexoraNotification("AIMBOT", "Enabled", 3, "target")
    else
        if AimbotConnection then AimbotConnection:Disconnect(); AimbotConnection = nil end
        SendNexoraNotification("AIMBOT", "Disabled", 3, "target")
    end
end
...
do
    ...
    getgenv()._NinjaToggle = function()
        ...
        SendNexoraNotification("Ninja Step", NinjaEnabled and "Active" or "Inactive", 2, nil)
    end
...
local function GetGunNow()
    ...
    if found then
        SendNexoraNotification("GUN", "Gun acquired", 3, "check")
    else
        SendNexoraNotification("GUN", "No gun found", 3, "x")
    end
end
...
Tabs.Manager:Toggle({
    ...
    ["Callback"] = function(v)
        ...
        SendNexoraNotification("Shoot Setting", v and "Gun stays equipped" or "Gun unequips after shot", 2, nil)
    end
})
...
Tabs.Manager:Toggle({
    ...
    ["Callback"] = function(v)
        ...
        SendNexoraNotification("Aim Laser", v and "Laser enabled" or "Laser disabled", 2, nil)
    end
})
...
HideBtn.MouseButton1Click:Connect(function()
    ...
    if HiddenButtons[name] then
        ...
        HideBtn.Text = "HIDE THIS BUTTON"
        HideBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
        SendNexoraNotification("Quick Buttons", name .. " is now visible", 2, "eye")
    else
        ...
        HideBtn.Text = "SHOW THIS BUTTON"
        HideBtn.BackgroundColor3 = Color3.fromRGB(34, 139, 34)
        SendNexoraNotification("Quick Buttons", name .. " hidden", 2, "eye-off")
    end
    SaveConfig()
end)
...
local function GetRightLowerArmAttachment(character)
    if not character then return nil end

    -- Always use RightLowerArm as the fixed laser origin
    local lowerArm = character:FindFirstChild("RightLowerArm")
    if not lowerArm then return nil end

    -- Reuse existing attachment if already created
    local att = lowerArm:FindFirstChild("LaserStart")
    if not att then
        att = Instance.new("Attachment", lowerArm)
        att.Name = "LaserStart"
        att.Position = Vector3.new(0, 0, 0) -- center of RightLowerArm
    end
    return att, lowerArm
end
...
local function CreateAimLaser()
    ...
    -- Laser starts from RightLowerArm, not from gun part
    local startAtt, lowerArm = GetRightLowerArmAttachment(character)
    if not startAtt or not lowerArm then return end

    -- End attachment: parented to RightLowerArm as well (world-positioned each frame)
    local endAtt = lowerArm:FindFirstChild("LaserEnd")
    if not endAtt then
        endAtt = Instance.new("Attachment", lowerArm)
...
local function UpdateAimLaser()
    ...
    -- Recreate if RightLowerArm changed or beam is gone
    local lowerArm = character:FindFirstChild("RightLowerArm")
    if not lowerArm or not AimLaserBeam or AimLaserBeam.Parent ~= lowerArm then
        CreateAimLaser()
        return
    end
...
SPUnhideAll.MouseButton1Click:Connect(function()
    ...
    SaveConfig()
    RebuildSPList()
    SendNexoraNotification("Unhide All", count .. " button(s) restored", 2, nil)
end)
...
local SilentSettings = {
    ...
    -- V22 (classic)
    KnifePrediction = 0.165,
    GunPrediction = 0.02,
    MaxDistance = 500,
    -- V39 (advanced)
    ThrowMulti = 1.25,
    GravityAdjustment = 196.2,
    -- Mode controls
    AimMode = "Silent Aim",   -- "Silent Aim" | "Instant Throw"
    DetectMode = "Nearest"    -- "Nearest" | "Nearest Mouse"
}
...
AimVisualPart.CanCollide = false
AimVisualPart.CastShadow = false
AimVisualPart.Material = Enum.Material.Neon
AimVisualPart.Transparency = 1
...
local function MakeBracketFrames(parent, pos)
...
local aimBracketH, aimBracketV = MakeBracketFrames(AimLockGui, UDim2.new(0.4, 0, 0.4, 0))
local _ok = pcall(function() AimLockGui.Parent = game:GetService("CoreGui") end)
if not _ok then AimLockGui.Parent = workspace.CurrentCamera end

-- Role helper
local function GetSilentRole(Player)
...
RunService.RenderStepped:Connect(function()
    ...
    if SilentSettings.VisualEnabled and arm and CurrentTargetPart then
        ...
        local col = (role == "Murderer" and Color3.fromRGB(255, 50, 50))
                 or (role == "Sheriff"   and Color3.fromRGB(50, 130, 255))
                 or Color3.fromRGB(0, 255, 120)
        AimVisualPart.Color = col
        AimLockGui.Adornee = CurrentTargetPart
        AimLockGui.Enabled = true
        aimBracketH.BackgroundColor3 = col
        aimBracketV.BackgroundColor3 = col
    else
        AimVisualPart.Transparency = 1
        AimLockGui.Enabled = false
    end
        ...
        if myRoot and SilentSettings.Enabled then
            ...
            for _, v in pairs(Players:GetPlayers()) do
                if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild(SilentSettings.TargetPart) then
                    local part = v.Character[SilentSettings.TargetPart]
                    ...
                    if dist3D < SilentSettings.MaxDistance then
                        ...
                        local screenDist = onScreen
                            ...
                            or math.huge
                        table.insert(potential, {
                            Part = part,
                            Role = GetSilentRole(v),
                            Dist = dist3D,
            ...
            local iHaveGun   = myBP and (myBP:FindFirstChild("Gun")   or (myChar2 and myChar2:FindFirstChild("Gun")))

            local priorityRole = nil
            if iHaveKnife then
                priorityRole = "Sheriff"
            elseif iHaveGun then
                priorityRole = "Murderer"
            end

            -- First pass: try to find a priority target
            local target = nil
            if priorityRole then
                if SilentSettings.DetectMode == "Nearest Mouse" then
                    local best = math.huge
                    for _, d in pairs(potential) do
                        if d.Role == priorityRole and d.ScreenDist < best then
                            target = d.Part; best = d.ScreenDist
                        end
                    end
                else
                    local best = SilentSettings.MaxDistance
                    for _, d in pairs(potential) do
                        if d.Role == priorityRole and d.Dist < best then
                            target = d.Part; best = d.Dist
                        end
                    end
                end
            end

            -- Fallback: no priority target found, pick anyone by DetectMode
            if not target then
                if SilentSettings.DetectMode == "Nearest Mouse" then
                    local best = math.huge
                    for _, d in pairs(potential) do
                        if d.ScreenDist < best then target = d.Part; best = d.ScreenDist end
                    end
                else
                    local best = SilentSettings.MaxDistance
                    for _, d in pairs(potential) do
                        if d.Dist < best then target = d.Part; best = d.Dist end
                    end
                end
            end
...
-- Hook (AimMode-aware: Silent Aim = V22, Instant Throw = V39)
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if method == "FireServer"
        and (name == "KnifeThrown" or name == "Shoot")
    ...
    then
        ...
        if SilentSettings.AimMode == "Silent Aim" then
            ...
            if typeof(args[1]) == "CFrame" then
                args[1] = CFrame.lookAt(args[1].Position, predictedPos)
...
Tabs.Main:Toggle({
    ...
    ["Desc"] = "Role-aware aimbot: Murderer targets Sheriff, Sheriff targets Murderer. Covers knife throws and gun shots.",
    ["Value"] = false,
    ["Callback"] = function(state)
        SilentSettings.Enabled = state
        SendNexoraNotification("Silent Aim", state and "Aimbot Activated" or "Aimbot Deactivated", 2, state and "target" or "shield-off")
    end
})
...
Tabs.Main:Toggle({
    ...
    ["Callback"] = function(state)
        ...
        SendNexoraNotification("Aim Visual", state and "Visual Enabled" or "Visual Disabled", 2, state and "eye" or "eye-off")
    end
})
...
Tabs.Main:Dropdown({
    ...
    ["Callback"] = function(selected)
        SilentSettings.AimMode = selected
        SendNexoraNotification("Aim Mode", "Switched to: ", 2, "settings")
    end
})

Tabs.Main:Dropdown({
    ["Title"]  = "Detection Mode",
    ["Desc"]   = "Nearest: closest player in 3D space. Nearest Mouse: closest player to your cursor/tap.",
    ["Values"] = {"Nearest", "Nearest Mouse"},
    ["Value"]  = "Nearest",
    ["Multi"]  = false,
    ["Callback"] = function(selected)
        SilentSettings.DetectMode = selected
        SendNexoraNotification("Detection Mode", "Switched to: ", 2, "crosshair")
    end
})
...
Tabs.Main:Button({
    ...
    ["Callback"] = function()
        ...
        SendNexoraNotification("Camera", "FOV Reset to 70", 2, nil)
    end
})
...
do
    ...
    local function FA_activate()
        if not FA_hasKnife() then
            SendNexoraNotification("Freeze Aura", "Requires Knife in inventory!", 4, "x")
            return false
        end
        FA_active = true
    ...
    Tabs.Main:Toggle({
        ...
        ["Callback"] = function(State)
            if State then
                ...
                FA_deactivate()
                SendNexoraNotification("Freeze Aura", "Deactivated.", 3, "check")
            end
        end
    })
...
Tabs.ESP:Toggle({
    ...
    ["Callback"] = function(State)
        ...
        if State then
            ...
            SendNexoraNotification("Gun Notifier", "You'll be notified when a gun drops", 3, "bell")
        else
            StopGunAvailableNotif()
            SendNexoraNotification("Gun Notifier", "Disabled", 3, "bell-off")
        end
    end
})
    ...
    ["Buttons"] = {
        {
            ...
            ["Callback"] = function()
                ...
                SendNexoraNotification("TikTok Link Copied", "Successfully saved to clipboard!", 3, nil)
            end
        }
    }
})
    ...
    ["Buttons"] = {
        {
            ...
            ["Callback"] = function()
                ...
                SendNexoraNotification("YouTube Link Copied", "Successfully saved to clipboard!", 3, nil)
            end
        }
    }
})
